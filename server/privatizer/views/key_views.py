...
def api_list(request):
	...
	if userid != None:
		...
		return [{'hash': key.hash(), 'name': key.name, 'description': key.description, 
			'shared_with': [
			{'name': permission.user.user_name, 'id': permission.user_id} for permission in key.permissions
			]} for key in keys]
	else:
		return Response('Forbidden', 403)

@view_config(route_name='key.api.by_id', renderer='json', permission='auth')
def key_api_by_id(request):
	...
	if key:
		...
		if key.owner_id == auth_user:
			permissions = [ 
				{'name': permission.user.user_name, 'id': permission.user_id} 
