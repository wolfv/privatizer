...
def api(request):
    ...
    statsd.Counter('privatizer.api.request').increment()
...
def key_api_modify(context, request):
    ...
    if request.method == 'PUT':
        statsd.Counter('privatizer.api.key_modify').increment()
...
def key_api_new(request):
    if request.POST['name']:
        statsd.Counter('privatizer.api.key_new').increment()
        timer = statsd.Timer('privatizer.api.timer.key_new').start()
...
def key_api_permission(context, request):
    statsd.Counter('privatizer.api.modify_permissions').increment()
    timer = statsd.Timer('privatizer.api.timer.modify_permissions')
    timer.start()
    log.debug(context)
    key = context
    if request.method == 'POST':
        if request.json_body['name']:
            ...
            if user:
                user_id = user.id
                res = DBSession.query(KeyPermission).filter(
                            KeyPermission.user_id == user_id,
        ...
        timer.stop()
        return HTTPBadRequest('User not found. Please invite your friend to join!')
...
def key_api_delete(context, request):
    if request.method == 'DELETE':
        statsd.Counter('privatizer.api.delete_permission').increment()
