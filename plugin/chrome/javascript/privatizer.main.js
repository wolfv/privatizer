...
(function() {
  ...
  Privatizer = {
    ...
    login: function(username, password) {
      return sendRequest({
        ...
        onload: function(response) {
          ...
          return Privatizer.decryptDOM(true);
        }
      });
    },
    /*
    ...
    decryptDOM: function(refresh) {
      ...
      for (_j = 0, _len1 = messages.length; _j < _len1; _j++) {
        ...
        _results.push((function() {
          msg.textContent.replace(preg, function() {
            if (RegExp.$1) {
              return Crypt.decrypt(msg, RegExp.$3, RegExp.$2);
            }
          });
          if (refresh && msg.oldText !== void 0) {
            return msg.oldText.replace(preg, function() {
              if (RegExp.$1) {
                return Crypt.decrypt(msg, RegExp.$3, RegExp.$2);
              }
            });
    ...
    */
    ...
    listKeysPopup: function(elem) {
      var curHash, padlock, request;
      ...
      return request = sendRequest({
        ...
        url: purl + "api/keys",
        onload: function(response) {
          var info, json, key, ul, _fn, _i, _len;
    ...
    */
    ...
    decrypt: function(msg, value, keyhash) {
      return sendRequest({
        url: purl + "api/key/" + keyhash,
        onload: function(response) {
          var cryptobutton, decrypted, decryptedText, json;
          if (response.status === 200) {
            try {
              json = JSON.parse(response.text);
          ...
          if (decrypted) {
            ...
            cryptobutton.innerHTML = "[Y]";
          }
          msg.innerHTML = decryptedText + " ";
          msg.appendChild(cryptobutton);
          cryptobutton.onclick = function(e) {
            var fillFunction;
  ...
  Popup = (function() {
    ...
    function Popup() {
      ...
      this.PopupElement.id = 'privatizer-popup';
      this.PopupElement.className = 'privatizer-popup visible';
    ...
    Popup.prototype.destroy = function() {
      return document.body.removeChild(this.PopupElement);
    };

    Popup.prototype.PopupElement = null;

    Popup.prototype.atElem = null;

    Popup.prototype.isOpen = false;

    Popup.prototype.curFillFunction = false;

    Popup.prototype.close = function(e) {
      var checktarget, elem;
      if (!e) {
        if (this.isOpen) {
          this.atElem.setAttribute('open', '0');
          try {
            this.PopupElement.className = 'privatizer-popup hidden';
            this.PopupElement.style.display = 'none';
            this.isOpen = false;
      ...
      if (this.isOpen && e.target !== this.atElem && checktarget(e.target)) {
        this.atElem.setAttribute('open', '0');
        try {
          this.PopupElement.className = 'privatizer-popup hidden';
          this.PopupElement.style.display = 'none';
    ...
    Popup.prototype.open = function(atElem, fillFunction) {
      var offset;
      if (this.isOpen && this.atElem === atElem) {
        return;
      }
      this.atElem = atElem;
      this.curFillFunction = fillFunction;
      offset = DOM.totalOffset(this.atElem);
      this.PopupElement.style.left = offset['x'] + "px";
      this.PopupElement.style.top = offset['y'] + 30 + "px";
      this.PopupElement.style.display = 'block';
      this.PopupElement.innerHTML = '';
      fillFunction(this.PopupElement);
      document.addEventListener('click', function(e) {
        window.privatizer.popup.close(e);
        return true;
      }, false);
      this.isOpen = true;
      return this.atElem.setAttribute('open', '1');
    };
